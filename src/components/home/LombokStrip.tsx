...
export default function LombokStrip() {
  return (
    <section className="relative bg-cream overflow-hidden px-[5.5vw] py-[120px]">
      {/* Watermark */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none select-none">
        <span className="font-serif text-[20vw] text-black/[0.03] whitespace-nowrap uppercase tracking-widest">
          Lombok
        </span>
      </div>
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20">
        ...
        <RevealWrapper>
          <div>
            ...
            <h2 className="font-serif text-black text-3xl md:text-4xl lg:text-[2.75rem] leading-[1.2] mb-6">
              Why Lombok,
              <br />
              <em className="text-taupe">Why Now</em>
            </h2>
            <p className="text-black/60 text-base font-light leading-relaxed max-w-md">
        ...
        <div className="space-y-0">
          {facts.map((fact, i) => (
            <RevealWrapper key={fact.title} delay={i * 0.1}>
              <div className="border-t border-black/10 py-7 flex gap-5">
                <div className="w-11 h-11 border border-black/15 flex items-center justify-center flex-shrink-0">
                  <fact.icon size={18} className="text-taupe" />
                </div>
                <div>
                  <h3 className="font-serif text-black text-base mb-1.5">
                    {fact.title}
                  </h3>
